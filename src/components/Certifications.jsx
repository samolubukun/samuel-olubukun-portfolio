...
const Certifications = () => {
    ...
    return (
        <div className="border-b border-neutral-200 dark:border-neutral-900 pb-24 transition-colors duration-300">
            ...
            {/* Desktop/Tablet View - Full Cards but optimized for 5 in a row */}
            <div className="hidden md:grid md:grid-cols-3 lg:grid-cols-5 gap-4 px-4">
                {CERTIFICATIONS.map((cert, index) => (
                    <motion.div
                        ...
                        key={index}
                        className="w-full bg-neutral-100/50 dark:bg-neutral-900/50 p-4 rounded-2xl border border-neutral-200 dark:border-neutral-800 hover:border-blue-500/50 hover:shadow-lg dark:hover:shadow-blue-500/20 transition-all hover:-translate-y-2 flex flex-col justify-between relative overflow-hidden"
                    >
                        <span className="absolute top-2.5 right-2.5 text-[10px] font-semibold text-neutral-600 dark:text-neutral-500 bg-neutral-200 dark:bg-neutral-800 px-1.5 py-0.5 rounded border border-neutral-300 dark:border-neutral-700 z-10">
                            {cert.year}
                        </span>

                        <div className="flex flex-col items-center">
                            <div className={`w-24 h-24 bg-white dark:bg-neutral-800 rounded-lg flex items-center justify-center overflow-hidden border border-neutral-200 dark:border-neutral-700 mb-4 shadow-inner ${cert.issuer === 'DataCamp' ? 'p-1.5' : 'p-0'}`}>
                                {cert.badge ? (
                                    <img
                                        src={cert.badge}
                                        alt={`${cert.title} badge`}
                            ...
                            <h3 className="text-sm xl:text-base font-semibold text-neutral-900 dark:text-neutral-200 mb-1 leading-tight text-center">
                                {cert.title.split("\n").map((line, i) => (
                                    <span key={i} className="block">{line}</span>
                                ))}
                            </h3>
                            <p className="text-neutral-600 dark:text-neutral-400 text-xs mb-4 text-center">
                                {cert.issuer}
                            </p>
                        </div>
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center text-xs xl:text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 transition-colors mt-auto mx-auto group"
                        >
                            View Credential <FaExternalLinkAlt className="ml-1.5 text-[10px] group-hover:translate-x-1 transition-transform" />
                        </a>
                    </motion.div>
                ))}
            </div>
